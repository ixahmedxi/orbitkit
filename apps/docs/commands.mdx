...
You can start up for example only the web application by using `bun turbo dev --filter=web`.
...
You can build for example only the web application by using `bun turbo build --filter=web`.
